...
switch ($metodo) {
    case 'GET':
        ...
        foreach ($agendas as $a)
            $mapa[(int) $a['id']] = $a['titulo'] ?? '';
        foreach ($lembretes as &$l)
            $l['agenda_titulo'] = $mapa[(int) ($l['agenda_id'] ?? 0)] ?? '-';
        unset($l);
    ...
    case 'POST':
        ...
        if (salvar($dataFile, $dados))
            responder(['id' => $novoId, 'mensagem' => 'Lembrete criado']);
        else
        ...
        foreach ($dados['lembretes'] as $i => $l) {
            if ((int) $l['id'] === $id) {
                if (isset($body['mensagem']))
                    $dados['lembretes'][$i]['mensagem'] = trim($body['mensagem']);
                if (isset($body['data_lembrete']))
                    $dados['lembretes'][$i]['data_lembrete'] = trim($body['data_lembrete']);
                if (isset($body['lido']))
                    $dados['lembretes'][$i]['lido'] = (bool) $body['lido'];
                if (isset($body['agenda_id']))
                    $dados['lembretes'][$i]['agenda_id'] = (int) $body['agenda_id'];
                if (salvar($dataFile, $dados))
                    responder(['mensagem' => 'Lembrete atualizado']);
                else
                    responder(['erro' => 'Erro ao salvar'], 500);
                exit;
            }
